...
/**
 * A mod_cpassignment adhoc task to fetch back transcriptions from Amazon S3
 *
 * @package    mod_cpassignment
 * @since      Moodle 2.7
 * @copyright  2015 Justin Hunt (poodllsupport@gmail.com)
     ...
     */
	 public function execute(){
		...
		//$trace->output($cd->somedata)
         ...
         if(!\mod_cpassignment\utils::can_transcribe($activity)){
             $this->do_forever_fail('This activity does not support transcription',$trace);
             return;
         }

         $submission = new \mod_cpassignment\submission($cd->attemptid,$cd->modulecontextid);
         if(!$submission){
         ...
         $transcript = \mod_cpassignment\utils::curl_fetch($transcripturl,false);
         if(!$transcript){
             $this->do_retry_fail('Transcript appears to not be ready yet',$trace);
             return;
         }else{
             $fulltranscript = \mod_cpassignment\utils::curl_fetch($fulltranscripturl,false);
            // $subtitles = \mod_cpassignment\utils::curl_fetch($subtitleurl,false);
             $record= new \stdClass();
