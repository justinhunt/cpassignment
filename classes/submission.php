...
class submission{
	...
	function __construct($attemptid, $modulecontextid=0) {
	   ...
	   if($attemptdata){
			...
			$this->activitydata = $DB->get_record(constants::M_TABLE,array('id'=>$attemptdata->cpassignmentid));
		}
   }
   ...
   public function prepare_javascript($reviewmode=false,$aimode=false){
       ...
       $gradingopts['opts_id'] = 'mod_cpassignment_submissionopts';
       ...
       $PAGE->requires->js_call_amd("mod_cpassignment/gradenowhelper", 'init', array(array('id'=>$gradingopts['opts_id'])));
