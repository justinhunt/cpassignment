 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class itemid extends \core\output\inplace_editable {
    /**
     * Constructor.
     *
    ...
    public function __construct($item) {
        ...
        $displayvalue = format_string($item->{constants::LIST_ITEM_ID} , true, array('context' => $coursecontext));
        parent::__construct(constants::M_COMP, 'itemid', $item->id, $editable,
            $displayvalue,
                $item->{constants::LIST_ITEM_ID},
            new lang_string('edititemid', constants::M_COMP),
    ...
    public static function update($itemid, $newvalue) {
        ...
        if($item){
            $item->itemid=$item->{constants::LIST_ITEM_ID};
        }
        $coursecontext = \context_course::instance($item->courseid);
        ...
        if (strval($newvalue) !== '') {
            $upd = new \stdClass();
            $upd->id=$itemid;
            $upd->{constants::LIST_ITEM_ID} = $newvalue;
            $DB->update_record(constants::M_USERTABLE, $upd);
            $item->{constants::LIST_ITEM_ID} =$newvalue;
