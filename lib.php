...
/**
 * Library of interface functions and constants for module cpassignment
 *
 * All the core Moodle functions, neeeded to allow the module to work
 * integrated in Moodle should be placed here.
 * All the cpassignment specific functions, needed to implement all the module
 ...
 * whether the course reset functionality affects the cpassignment.
 *
 * @param $mform form passed by reference
 */
function cpassignment_reset_course_form_definition(&$mform) {
 ...
 * @return array
 */
function cpassignment_reset_course_form_defaults($course) {
 ...
 */
function cpassignment_reset_gradebook($courseid, $type='') {
    global $CFG, $DB;
    ...
    if ($moduleinstances = $DB->get_records_sql($sql,$params)) {
        foreach ($moduleinstances as $moduleinstance) {
            cpassignment_grade_item_update($moduleinstance, 'reset');
        }
    }
}
 ...
 * cpassignment attempts for course $data->courseid.
 *
 * @global stdClass
 * @global object
 ...
 */
function cpassignment_reset_userdata($data) {
    global $CFG, $DB;
    ...
    if (!empty($data->{'reset_' . constants::M_MODNAME})) {
        ...
        if (empty($data->reset_gradebook_grades)) {
            cpassignment_reset_gradebook($data->courseid);
        }
 ...
 */
function cpassignment_grade_item_update($moduleinstance, $grades=null) {
    global $CFG;
 ...
 */
function cpassignment_update_grades($moduleinstance, $userid=0, $nullifnone=true) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/gradelib.php');

    if ($moduleinstance->grade == 0) {
        cpassignment_grade_item_update($moduleinstance);

    } else if ($grades = cpassignment_get_user_grades($moduleinstance, $userid)) {
        cpassignment_grade_item_update($moduleinstance, $grades);

    } else if ($userid and $nullifnone) {
        $grade = new stdClass();
        $grade->userid   = $userid;
        $grade->rawgrade = null;
        cpassignment_grade_item_update($moduleinstance, $grade);
 ...
 */
function cpassignment_get_user_grades($moduleinstance, $userid=0) {
    global $CFG, $DB;
    ...
    if ($moduleinstance->maxattempts==1 || $moduleinstance->gradeoptions == constants::M_GRADELATEST) {
		...
		$sql = "SELECT u.id, u.id AS userid, a.sessionscore AS rawgrade
                     ...
                     WHERE a.id= (SELECT max(id) FROM {". constants::M_USERTABLE ."} ia WHERE ia.userid=u.id AND ia.cpassignmentid = $idfield)  AND u.id = a.userid AND $idfield = :moduleid
                           $user
...
function cpassignment_get_completion_state($course,$cm,$userid,$type) {
	return cpassignment_is_complete($course,$cm,$userid,$type);
}


//this is called internally only
function cpassignment_is_complete($course,$cm,$userid,$type) {
	 global $CFG,$DB;
 ...
 *
 */
function cpassignment_dotask(progress_trace $trace) {
...
/**
 * Saves a new instance of the cpassignment into the database
 *
...
/**
 * Updates an instance of the cpassignment in the database
 *
 * Given an object containing all the necessary data,
 * (defined by the form in mod_form.php) this function
...
/**
 * Removes an instance of the cpassignment from the database
 *
 ...
 */
function cpassignment_delete_instance($id) {
    global $DB;
 ...
 */
function cpassignment_user_outline($course, $user, $mod, $themodule) {
 ...
 */
function cpassignment_user_complete($course, $user, $mod, $themodule) {
}

/**
 * Given a course and a time, this module should find recent activity
 * that has occurred in cpassignment activities and print it out.
 * Return true if there was output, or false is there was none.
 *
 * @return boolean
 */
function cpassignment_print_recent_activity($course, $viewfullnames, $timestart) {
 ...
 * {@link cpassignment_print_recent_mod_activity()}.
 *
 ...
 */
function cpassignment_get_recent_mod_activity(&$activities, &$index, $timestart, $courseid, $cmid, $userid=0, $groupid=0) {
}

/**
 * Prints single activity item prepared by {@see cpassignment_get_recent_mod_activity()}

 * @return void
 */
function cpassignment_print_recent_mod_activity($activity, $courseid, $detail, $modnames, $viewfullnames) {
}
 ...
 * @return array
 */
function cpassignment_get_extra_capabilities() {
    return array();
}
...
/**
 * Is a given scale used by the instance of cpassignment?
 *
 * This function returns if a scale is being used by one cpassignment
 * if it has support for grading and scales. Commented code should be
 * modified if necessary. See forum, glossary or journal modules
 * as reference.
 *
 * @param int $themoduleid ID of an instance of this module
 * @return bool true if the scale is used by the given cpassignment instance
 */
function cpassignment_scale_used($themoduleid, $scaleid) {
    global $DB;
...
/**
 * Checks if scale is being used by any instance of cpassignment.
 *
 * This is used to find out if scale used anywhere.
 *
 * @param $scaleid int
 * @return boolean true if the scale is used by any cpassignment instance
 */
function cpassignment_scale_used_anywhere($scaleid) {
    global $DB;
 ...
 */
function cpassignment_get_file_areas($course, $cm, $context) {
    return cpassignment_get_editornames();
}

/**
 * File browsing support for cpassignment file areas
 *
 * @package mod_cpassignment
 * @category files
 *
 ...
 */
function cpassignment_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
    return null;
}

/**
 * Serves the files from the cpassignment file areas
 *
 * @package mod_cpassignment
 * @category files
 *
 ...
 * @param stdClass $context the cpassignment's context
 * @param string $filearea the name of the file area
 * @param array $args extra arguments (itemid, path)
 * @param bool $forcedownload whether or not force download
 * @param array $options additional options affecting the file serving
 */
function cpassignment_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
       global $DB, $CFG;
    ...
    if (!has_capability('mod/cpassignment:view', $context)) {
        ...
        $fullpath = "/$context->id/mod_cpassignment/$filearea/$itemid/$relativepath";
...
/**
 * Extends the global navigation tree by adding cpassignment nodes if there is a relevant content
 *
 * This can be called by an AJAX request so do not rely on $PAGE as it might not be set up properly.
 *
 * @param navigation_node $navref An object representing the navigation tree node of the cpassignment module instance
 * @param stdClass $course
 * @param stdClass $module
 * @param cm_info $cm
 */
function cpassignment_extend_navigation(navigation_node $navref, stdclass $course, stdclass $module, cm_info $cm) {
}

/**
 * Extends the settings navigation with the cpassignment settings
 *
 * This function is called when the context for the page is a cpassignment module. This is not called by AJAX
